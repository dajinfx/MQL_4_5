...
double points_below_market = 200; // Distance in points below current price for Sell Stop

void OnStart()
{
   double bid = SymbolInfoDouble(Symbol(), SYMBOL_BID);
   double sellstop_price = NormalizeDouble(bid - points_below_market * Point(), Digits());
   bool od_result = Od_Send(Symbol(), ORDER_TYPE_SELL_STOP, Lots, sellstop_price, loss_dis_p, profit_dis_p, "Scripts_mql5_sellstop", magic_no);
}
